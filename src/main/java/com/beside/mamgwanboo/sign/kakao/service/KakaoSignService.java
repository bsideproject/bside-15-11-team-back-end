...
public class KakaoSignService extends AbstractSignService {
	...
	@Override
	public Mono<UserInformation> getUserInformation(OauthServiceType oauthServiceType, String accessToken) {
		return webClient
			.get()
			.uri(apiUri)
	...
	public Mono<User> getUser(OauthServiceType oauthServiceType, String serviceUserId, YnType useYn) {
		return userRepository.findUserByUserInformation_OauthServiceTypeAndUserInformation_ServiceUserIdAndUseYn(oauthServiceType, serviceUserId, useYn);
	}
	...
	private UserInformation toUserInformation(OauthServiceType oauthServiceType, KakaoApiResponse kakaoApiResponse) {
		return UserInformation.builder()
			...
			.birth(Birth.builder()
				...
				.date(toLocalDate(kakaoApiResponse.getKakaoAccount().getBirthYear(), kakaoApiResponse.getKakaoAccount().getBirthDay()))
				.build())
			.build();
	}
